...
/* ================= VIDEO CARD ================= */
const VideoCard = ({ video, reportId }) => {
  const [isPlaying, setIsPlaying] = useState(false);
          ...
          events: {
            onReady: (event) => {
              if (video.watchTime > 0) {
                event.target.seekTo(video.watchTime);
              }
            },
            onStateChange: (event) => {
  ...
  return (
    <motion.div className="bg-slate-900 rounded-2xl overflow-hidden shadow-lg ">
      <div className="aspect-video bg-black relative w-full">
        {isPlaying ? (
          <div id={`player-${video._id}`}  className="w-full h-full" />
        ) : (
          <div
            onClick={() => setIsPlaying(true)}
  ...
  const fetchVideos = async () => {
    try {
      ...
      const res = await fetch(
        `/api/get-user-tests?email=${user.email}`
      );
      const data = await res.json();

      const groups = data.tests.map((test) => ({
        id: test._id,
        subject: test.technicalAssessment?.subject,
        date: new Date(test.createdAt).toLocaleDateString("mr-IN"),
        videos: test.aiReport.videoSuggestions,
      }));
